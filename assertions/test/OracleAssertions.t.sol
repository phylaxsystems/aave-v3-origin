...
contract OracleAssertionsTest is CredibleTest, Test, TestnetProcedures {
    ...
    function test_assertionBorrowPriceDeviation() public {
        ...
        cl.validate(
            ...
            address(pool),
            0,
            abi.encodeWithSelector(pool.borrow.selector, asset, 100e6, DataTypes.InterestRateMode.VARIABLE, 0, alice)
        );
        vm.stopPrank();
    }
    ...
    function test_assertionSupplyPriceDeviation() public {
        ...
        // This should pass because price hasn't deviated
        cl.validate(
            ASSERTION_LABEL, address(pool), 0, abi.encodeWithSelector(pool.supply.selector, asset, 100e6, alice, 0)
        );
        vm.stopPrank();
    }
}
