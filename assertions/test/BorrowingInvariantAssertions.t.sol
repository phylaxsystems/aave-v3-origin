...
contract TestBorrowingInvariantAssertions is CredibleTest, Test, TestnetProcedures {
    ...
    function setUp() public {
        ...
        (,, address variableDebtUSDX) = contracts.protocolDataProvider.getReserveTokensAddresses(asset);
    ...
    function test_assertionLiabilityDecrease() public {
        ...
        cl.addAssertion(
            ASSERTION_LABEL, address(pool), type(BorrowingPostConditionAssertions).creationCode, abi.encode(pool)
        );
        ...
        cl.validate(
            ...
            address(pool),
            0,
            abi.encodeWithSelector(pool.repay.selector, asset, repayAmount, DataTypes.InterestRateMode.VARIABLE, user)
        );
        vm.stopPrank();
    }
    ...
    function test_liabilityDecrease_regular() public {
        ...
        assertTrue(preDebt - postDebt >= repayAmount, "Debt decrease should be at least the repay amount");
    ...
    function test_assertionUnhealthyBorrowPrevention() public {
        ...
        cl.validate(
            ...
            address(pool),
            0,
            abi.encodeWithSelector(pool.borrow.selector, asset, 100e6, DataTypes.InterestRateMode.VARIABLE, 0, user)
        );
        vm.stopPrank();
    }
