...
contract HealthFactorAssertions is Assertion {
    ...
    function triggers() public view override {
        ...
        registerCallTrigger(this.assertLiquidationUnsafeBeforeAfter.selector, pool.liquidationCall.selector);
        registerCallTrigger(
            this.assertSetUserUseReserveAsCollateral.selector, pool.setUserUseReserveAsCollateral.selector
        );
    ...
    function assertNonDecreasingHfActions() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            (,, address onBehalfOf,) = abi.decode(callInputs[i].input, (address, uint256, address, uint16));
            ...
            require(postHealthFactor >= preHealthFactor, "Health factor decreased in non-decreasing action");
        }
    }
    ...
    function assertNonIncreasingHfActions() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(postHealthFactor <= preHealthFactor, "Health factor increased in non-increasing action");
        }
    }
    ...
    function assertHealthyToUnhealthy() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            if (preHealthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
                require(postHealthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Healthy account became unhealthy");
            }
        }
    }
    ...
    function assertUnsafeBeforeAction() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            (,, address user,,) = abi.decode(callInputs[i].input, (address, address, address, uint256, bool));
            ...
            if (preHealthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
                ...
                bytes4 currentSelector = bytes4(callInputs[i].input);
                require(currentSelector == pool.liquidationCall.selector, "Action on unsafe position not allowed");
            }
        }
    }
    ...
    function assertSupplyNonDecreasingHf() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(
                healthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Supply operation resulted in unhealthy position"
            );
        }
    }
    ...
    function assertBorrowHealthyToUnhealthy() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(
                healthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Borrow operation resulted in unhealthy position"
            );
        }
    }
    ...
    function assertWithdrawNonIncreasingHf() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(
                healthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Withdraw operation resulted in unhealthy position"
            );
        }
    }
    ...
    function assertRepayNonDecreasingHf() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(
                healthFactor >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Repay operation resulted in unhealthy position"
            );
        }
    }
    ...
    function assertLiquidationUnsafeBeforeAfter() external {
        ...
        for (uint256 i = 0; i < callInputs.length; i++) {
            ...
            require(preHealthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD, "Cannot liquidate healthy position");
    ...
    function assertSetUserUseReserveAsCollateral() external {
        PhEvm.CallInputs[] memory callInputs =
            ph.getCallInputs(address(pool), pool.setUserUseReserveAsCollateral.selector);
        for (uint256 i = 0; i < callInputs.length; i++) {
