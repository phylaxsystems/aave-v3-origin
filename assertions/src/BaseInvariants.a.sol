...
contract BaseInvariants is Assertion {
    ...
    function assertDebtTokenSupply() external {
        ...
        PhEvm.CallInputs[] memory liquidationCalls = ph.getCallInputs(address(pool), pool.liquidationCall.selector);
        ...
        for (uint256 i = 0; i < borrowCalls.length; i++) {
            (address borrowAsset, uint256 amount,,,) =
                abi.decode(borrowCalls[i].input, (address, uint256, uint256, uint16, address));
        ...
        for (uint256 i = 0; i < repayCalls.length; i++) {
            (address repayAsset, uint256 amount,,) =
                abi.decode(repayCalls[i].input, (address, uint256, uint256, address));
            if (repayAsset == asset) {
                balanceChange -= int256(amount);
            }
        }
        ...
        for (uint256 i = 0; i < liquidationCalls.length; i++) {
            (address collateralAsset, address debtAsset, address user, uint256 debtToCover,) =
                abi.decode(liquidationCalls[i].input, (address, address, address, uint256, bool));
            if (debtAsset == asset) {
    ...
    function _verifyDebtTokenSupplyChange(int256 expectedChange) internal {
        ...
        require(actualChange == expectedChange, "Debt token supply change does not match individual balance changes");
    }
}
